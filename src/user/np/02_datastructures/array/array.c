...
char array_remove(Array* a, int idx)
{
    if (idx > a->size || idx < 0)
    {
        printf("idx %i is out of range", idx);
        return '\0';
    }

    // Copy all elements after idx to 
    //the left, effectively overwriting idx
    char ret = a->elements[idx];
    for (int i = a->size; i > idx; --i)
    {
        a->elements[i - 1] = a->elements[i];
    }
    a->size--;
    int resizecap = a->capacity /2;
    if(a->size < resizecap){
        array_resize(a, resizecap);
    }
    return ret;
}
#if defined(__GNUC__) || defined(__clang__)
